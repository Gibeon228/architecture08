...
public class BookingPresenter implements ViewObserver {
    ...
    @Override
    public void onChangeReservationTable(int id, Date oldDate, int tableNo, String name) {
        try {
            int reservationNo = model.changeReservationTable(id, oldDate, tableNo, name);
            updateUIShowReservationResult(reservationNo);
        } catch (RuntimeException e) {
            updateUIShowReservationResult(-1);
        }
